...
class Mobilev2Block(nn.Module):
    ...
    def forward(self, input, pool_size=(2, 2), pool_type='avg'):
        ...
        x = self.conv1a(F.gelu(self.bn1a(origin)))
        x = self.conv1b(F.gelu(self.bn1b(x)))
        x = self.conv1c(F.gelu(self.bn1c(x)))
        ...
        x = self.conv2a(F.gelu(self.bn2a(origin)))
        x = self.conv2b(F.gelu(self.bn2b(x)))
        x = self.conv2c(F.gelu(self.bn2c(x)))
...
class Cnn14_mobilev2(nn.Module):
    def __init__(self, classes_num):
        ...
        # self.channel_attention = ChannelAttention(512, reduction_ratio=2)
    ...
    def forward(self, x):
        ...
        x = F.dropout(x, p=0.2, training=self.training)
        # chan_att = self.channel_attention(x) * x
        # spat_att = self.spatial_attention(chan_att) * chan_att
        # x = spat_att + identy
